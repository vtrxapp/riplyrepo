  ...
  return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column', position:'relative', background:C.pageBg, fontFamily:"'Montserrat',-apple-system,sans-serif" }}>
      ...
      {(role==='organizer' || role==='admin') && (
        <button type="button" onClick={()=>navigate('create-event')} aria-label="Create event" style={{ position:'absolute', bottom:94, right:18, width:60, height:60, border:'none', borderRadius:'50%', background:C.grad, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', boxShadow:'0 10px 24px rgba(2,162,240,0.45)', zIndex:6 }}>
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M12 5v14M5 12h14" stroke="#fff" strokeWidth="2.4" strokeLinecap="round"/></svg>
        </button>
      )}
    </div>
  );
...
function ProfileScreen({ navigate, showToast, currentUser, saved }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // Whitelisted to the same active-membership roles used elsewhere in
      // this file (e.g. GroupAnalyticsScreen's member count, admin checks)
      // rather than blacklisting 'pending' -- excludes pending join requests
      // (a request-only group hasn't actually approved this user yet) without
      // silently counting some future non-membership role no one's added yet.
      supabase.from('group_members').select('group_id', { count: 'exact', head: true }).eq('user_id', cu.userId).in('role', ['member', 'admin', 'owner']),
